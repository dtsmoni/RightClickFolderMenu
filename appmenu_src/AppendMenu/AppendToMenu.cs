...
namespace AppendMenu
{
    ...
    public class AppendToMenu : System.Windows.Forms.Form
    {
        ...
        /// </summary>
        [STAThread]
        static void Main() { Application.Run(new AppendToMenu()); }
        ...
        private void buttonSelect_Click(object sender, System.EventArgs e)
        {
            if (sender == this.buttonSelect)
            {
                ...
                if (openFileDialogSelectExe.ShowDialog(this) == DialogResult.OK) { this.txtPath.Text = openFileDialogSelectExe.FileName; }
            }
            if (sender == this.btnExit) { this.Close(); }
        }
        ...
        private void btnAddMenu_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (regmenu != null) { regmenu.SetValue("", this.txtName.Text); }
                regcmd = Registry.ClassesRoot.CreateSubKey(Command);
                if (regcmd != null) { regcmd.SetValue("", this.txtPath.Text); }
            }
        ...
        private void AppendToMenu_Load(object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                if (regmenu != null) { this.txtName.Text = (String)regmenu.GetValue(""); }
                regcmd = Registry.ClassesRoot.OpenSubKey(Command, false);
                if (regcmd != null) { this.txtPath.Text = (String)regcmd.GetValue(""); }
        ...
        private void txtPath_TextChanged(object sender, System.EventArgs e)
        {
            if (((TextBox)sender).Text.Length > 0)
            {
                ...
                {
                    if (txtPath.Text.IndexOf('\\') >= 0) { this.btnAddMenu.Enabled = this.txtName.Text.Length > 0 && File.Exists(txtPath.Text); }
                    else { this.btnAddMenu.Enabled = this.txtName.Text.Length > 0; }
                }
            }
            else { this.btnAddMenu.Enabled = false; }
            btnRemoveMenu.Enabled = txtName.Text.Length > 0 || this.txtPath.Text.Length > 0;
        }
